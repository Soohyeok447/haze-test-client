...
function handleRestartMatchingRequest() {
  ...
  socket.emit(MATCHING_EVENTS.START_MATCHING, { userId });
}
...
function handlePartnerDisconnected() {
  ...
  socket.emit(MATCHING_EVENTS.START_MATCHING, { userId }); // 소개매칭 재요청
}
...
async function handleStartWebrtcSignaling({ roomName, stunServers }) {
  ...
  socket.emit(WEBRTC_EVENTS.OFFER, { offer, roomName });
...
async function handleOffer({ offer, roomName }) {
  ...
  socket.emit(WEBRTC_EVENTS.ANSWER, { answer, roomName });
...
function handleIceCandidate(data) {
  ...
  socket.emit(WEBRTC_EVENTS.ICE, { ice: data.candidate, roomName: room });
